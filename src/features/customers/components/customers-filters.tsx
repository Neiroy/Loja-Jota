...
function normalizeFilters(search: string, status: CustomerStatusFilter) {
  return {
    search: search ?? '',
    status: status ?? 'all',
  };
...
export function CustomersFilters({ search, status }: CustomersFiltersProps) {
  ...
  const [statusValue, setStatusValue] = useState<CustomerStatusFilter>(
    normalized.status
  );
  ...
  function applyFilters(nextSearch: string, nextStatus: CustomerStatusFilter) {
    ...
    <FilterPanel onSubmit={handleSubmit}>
      ...
      <div className="w-full space-y-2 sm:w-44">
        ...
        <select
          ...
          value={statusValue}
          onChange={(event) =>
            setStatusValue(event.target.value as CustomerStatusFilter)
          }
          disabled={isPending}
